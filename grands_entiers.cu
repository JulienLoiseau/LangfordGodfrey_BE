 ...
 */
__host__ __device__ bool LONG_estNul(LONG *a) {
...
__host__ __device__ void LONG_recale(LONG *a) {
  for (int i=0 ; i < NB_MAX_MOTS-1 ; i++) { /* a->nbMots */
 ...
 * A utiliser après les LONG_multiplie_char, et la plupart des LONG_ajoute_LONG
 */
__host__ __device__ bool LONG_modulo(LONG *a, int nbBits) {
  /* hyp : a est recalePlus et est de quantite positive
   *       nbBits >= 0
   */

  int ecartIndex = nbBits / TAILLE_BASE ;
  nbBits %= TAILLE_BASE ;

  if ( ecartIndex >= a->nbMots )
    return ( LONG_estNul(a) ) ;

  for (int i=0 ; i < ecartIndex ; i++)
    if ( a->sequence[i] != 0 )
      return false ;
  ...
  for (int i = a->nbMots ; i < a->nbMots + ecartIndex ; i++ )
    a->sequence[i] = 0;

  /* 2 : on doit verifier qu'il y a suffisamment de '0' a droite */
  long diviseur = localPow(2,nbBits) ;
